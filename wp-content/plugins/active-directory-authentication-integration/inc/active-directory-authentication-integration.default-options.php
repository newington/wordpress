...
$default_options_info = array(
	...
	'adauthint_user_opts' 	=> array(
		...
		'append_ad_user_suffix'	=> array(
			'opt_type'		=> 'bool',
			'opt_default'	=> true,
			'opt_val'		=> $this->append_ad_user_suffix,
		...
		/**
		 * Whether to prepend an AD prefix before authenticating
		 * Should not be combined with append_ad_user_suffix
		 * @since 0.7
		 */
		'prepend_ad_user_prefix' => array(
			'opt_type'		=> 'bool',
			'opt_default'	=> true,
			'opt_val'		=> $this->prepend_ad_user_prefix,
			'opt_label'		=> __( 'Prepend server name to username before being validated?', ADAUTHINT_TEXT_DOMAIN ), 
			'opt_note'		=> __( 'If checked, the AD account suffix/prefix (see below) will be added to the beginning of usernames before they are checked against the AD server (in some cases, it is necessary to add a prefix like "EXAMPLE\\" to the username before it is authenticated).', ADAUTHINT_TEXT_DOMAIN ), 
		),
		/**
		...
		'ad_account_suffix'		=> array(
			...
			'opt_label'		=> __( 'AD Account Suffix/Prefix', ADAUTHINT_TEXT_DOMAIN ), 
			'opt_note'		=> __( 'If the suffix option above is enabled, the text here will be appended to all usernames before they are checked against the AD server (generally something like "@example.local"). If the prefix option above is enabled, the text here will be added to the beginning of the usernames before they are checked against the AD server (generally something like "EXAMPLE\\"', ADAUTHINT_TEXT_DOMAIN ), 
		),
		/**
		...
		'allow_local_password'	=> array(
			'opt_type'		=> 'bool',
			'opt_default'	=> false,
			'opt_val'		=> $this->allow_local_password,
