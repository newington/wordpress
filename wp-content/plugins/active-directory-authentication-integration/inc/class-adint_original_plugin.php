 ...
 */
	...
	protected function _set_adldap() {
		...
		$LDAP_Options = array(
			...
			'account_suffix'		=> $this->append_ad_user_suffix ? stripslashes( $this->ad_account_suffix ) : '',
			'account_prefix'		=> $this->prepend_ad_user_prefix ? stripslashes( $this->ad_account_suffix ) : '',
		);
		...
		if( $this->randomize_dc ) {
			...
			$this->_log( ADAI_LOG_INFO, "Options for adLDAP connection:\n" .
				...
				"'ad_username'			=> $this->bind_user, \n" . 
				"'ad_password'			=> " . ( $this->_mask_passwords_in_log ? md5( $this->bind_user_password ) : $this->bind_user_password ) . ", \n" .
				"'ad_port'				=> $this->port, \n" .
				"'use_tls'				=> $this->secure_connection, \n" . 
				"'use_ssl'				=> $this->use_ssl, \n" . 
				"'account_suffix'		=> " . ( $this->append_ad_user_suffix ? stripslashes( $this->ad_account_suffix ) : '' ) . ", \n" . 
	...
	function ldap_test() {
		$r = true;
		$this->_set_adldap();
		if( false === $this->_adldap ) {
			$r = false;
		} else {
			if( is_array( $this->_adldap ) ) {
				foreach( $this->_adldap as $a ) {
					if( false === $a )
						$r = false;
				}
			}
	...
	public function ad_authenticate( $user = NULL, $username = '', $password = '' ) {
		...
		if( false === $this->_adldap || ( is_array( $this->_adldap ) && in_array( false, $this->_adldap ) ) ) {
			...
			$tmp = ob_get_contents();
			ob_end_clean();
			$tmp = $this->_mask_passwords_in_log ? str_replace( $this->_ad_password, md5( $this->_ad_password ), $tmp ) : $tmp;
			$this->_log( ADAI_LOG_DEBUG, $tmp );
			unset( $tmp );
		...
		if( !$user || $user->user_login != $username ) {
			...
			if ( $this->auto_user_update || $notmember ) {
				...
				if( $this->auto_user_update ) {
					$userinfo = $this->_adldap->user_info( $ad_username, apply_filters( 'adai-ad-user-info', array( 'displayname', 'givenname', 'sn', 'samaccountname', 'cn', 'mail', 'description' ) ) );
					$userinfo = $userinfo[0];
					...
					$extra_fields = apply_filters( 'adai-user-meta-fields', array() );
					$user_id = $this->_update_user( $ad_username, $email, $first_name, $last_name, $display_name, $user_role, $extra_fields );
				} else {
	...
	protected function _log_flush() {
		...
		if( defined( 'WP_DEBUG_DISPLAY' ) && WP_DEBUG_DISPLAY ) {
			print( "[ADAI][0] Log Level set to " . $this->_loglevel . "\n" );
			print( $this->_debug );
		} else {
			error_log( "[ADAI][0] Log Level set to " . $this->_loglevel . "\n" );
			$this->_debug = explode( "\n\r", $this->_debug );
			foreach( $this->_debug as $d ) {
				error_log( $d );
			}
		}
		$this->_debug = '';
...
<div id="login">
	 ...
	 * @uses is_wp_error()
	 */
	protected function _update_user( $username, $email, $first_name, $last_name, $display_name = '', $role = '', $extra_fields = null ) {
	...
	function is_user_member_of_blog() {
		...
		if( $this->append_ad_user_suffix || $this->prepend_ad_user_prefix )
			$ad_username = str_replace( $this->ad_account_suffix, '', $ad_username );
	...
	public function disable_function() {
		...
		switch( $_GET['action'] ) {
			...
			case 'password_reset':
				do_action( 'adai-lost-password', $_GET['action'] );
