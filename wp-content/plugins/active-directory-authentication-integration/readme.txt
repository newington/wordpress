...
If you enable `WP_DEBUG` in your wp-config.php file, the debug level for this plugin will be automatically set to `ADAI_LOG_DEBUG`. You can still override that by using the `$ADAuthIntObj->setLogLevel()` function.

By default, all debug information output by this plugin will be sent to your server's error log. If you would like them to be printed to the screen, you can define the `WP_DEBUG_DISPLAY` constant in your wp-config.php file.
...
* Attempt to fix bug where passwords with apostrophes fail (thanks sanihipa)
* Fix issue with JavaScript file location (thanks @sleary)

= 0.7.1 =

* Modify the way the LDAP test function is run, so that it only occurs when a user is logging in/out, rather than on every page load
* Modify the way the plugin checks for the deprecated option table to avoid recurring errors/warnings

= 0.7 =

* Added option to prepend domain to beginning of AD usernames rather than appending domain info to the end of the usernames
* Modified logging function to flush information to the server log by default rather than the screen (which caused a debug screen to appear instead of redirecting to admin center on successful login)
* Modified `__construct()` and logging functions to obey `WP_DEBUG` and `WP_DEBUG_DISPLAY` constants
* Fixed two instances of unmasked AD password showing up in debug info
* Added two new filters (adai-ad-user-info and adai-user-meta-fields) to allow custom information to be retrieved from AD and added to user meta information
